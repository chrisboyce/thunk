...
use crate::note::{Content, Note};
use eframe::egui;
use eframe::egui::{Label, Response, Separator, Ui, Widget};
use std::ops::Deref;
...
impl NoteEditor {
    pub fn new(note: Box<Note>) -> Self {
        Self { note }
    }
    pub fn get_content(&self) -> &Vec<Content> {
        self.note.get_content()
    }
}
...
impl Widget for NoteEditor {
    fn ui(self, ui: &mut Ui) -> Response {
        let note = &self.note;
        for content in note.get_content().deref().iter() {
            ui.add(Label::new(content.to_string()));
        }
